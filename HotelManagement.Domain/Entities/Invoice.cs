...
namespace HotelManagement.Domain.Entities
{
    public class Invoice
    {
        ...
        {
            if (AmountPaid.HasValue && TotalAmount > 0)
            {
                BalanceDue = TotalAmount - AmountPaid.Value;
                // Ensure balance due does not go negative
                if (BalanceDue < 0)
                {
                    BalanceDue = 0;
                }
            ...
            {
                BalanceDue = TotalAmount; // If no payment has been made, balance due is the total amount
            }
        }

        /// <summary>
        /// Updates the invoice status based on its balance due.
        /// </summary>
        public void UpdateInvoiceStatus()
        {
            if (BalanceDue.HasValue && BalanceDue.Value <= 0.01M) // Use a small tolerance for floating point numbers
            {
